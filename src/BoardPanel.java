...
public class BoardPanel extends JPanel {
	...
	public TileType getTile(int x, int y) {
		TileType type = tiles[y * ROW_COUNT + x];
		return type;
	}
	...
	public void paintComponent(Graphics g) {
		...
		/**
		 * draws all the tiles, if they are initialized
		 */
		...
		for(int loopVariableA = 0; loopVariableA < COL_COUNT; loopVariableA++) {
			for(int loopVariableB = 0; loopVariableB < ROW_COUNT; loopVariableB++) {
				g.drawLine(loopVariableA * TILE_SIZE, 0, loopVariableA * TILE_SIZE, getHeight());
				g.drawLine(0, loopVariableB * TILE_SIZE, getWidth(), loopVariableB * TILE_SIZE);
			}
		}		
		 ...
		 * shows messages
		 */
		if(game.is_Game_Over() || game.is_New_Game() || game.is_Paused()) {
