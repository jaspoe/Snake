...
public class SnakeGame extends JFrame {
	 ...
	 */
	private LinkedList<Direction> directionsSnakeIsGoing;
			...
			public void keyPressed(KeyEvent e) {
				...
				case KeyEvent.VK_UP:
					if(!isPaused && !isGameOver) {
						if(directionsSnakeIsGoing.size() < MAX_DIRECTIONS) {
							Direction last = directionsSnakeIsGoing.peekLast();
							if(last != Direction.South && last != Direction.North) {
								directionsSnakeIsGoing.addLast(Direction.North);
							}
						}
					}
					break;
					...
					if(!isPaused && !isGameOver) {
						if(directionsSnakeIsGoing.size() < MAX_DIRECTIONS) {
							...
							if(last != Direction.North && last != Direction.South) {
								directionsSnakeIsGoing.addLast(Direction.South);
							}
						}
					}
					break;
					...
					if(!isPaused && !isGameOver) {
						if(directionsSnakeIsGoing.size() < MAX_DIRECTIONS) {
							...
							if(last != Direction.East && last != Direction.West) {
								directionsSnakeIsGoing.addLast(Direction.West);
							}
						}
					}
					break;
					...
					if(!isPaused && !isGameOver) {
						if(directionsSnakeIsGoing.size() < MAX_DIRECTIONS) {
							...
							if(last != Direction.West && last != Direction.East) {
								directionsSnakeIsGoing.addLast(Direction.East);
							}
						}
					}
					break;
				...
				case KeyEvent.VK_ENTER:
					...
					/*
					 * If it's not a new game or game over...
					 * 
	...
	private void startGame() {
		...
		this.directionsSnakeIsGoing = new LinkedList<>();
	...
	private TileType updateSnake() {
		 ...
		 * it will not move).
		 */
		Direction direction = directionsSnakeIsGoing.peekFirst();
		...
		if(old != TileType.SnakeBody) {
			...
			if(directionsSnakeIsGoing.size() > 1) {
				directionsSnakeIsGoing.poll();
			}
		}
	...
	/**
	 * cleares scores, and prepares the board for a new game
	 *  gameReset
	 */
	private void whatDoesItDo() {	
		this.score = 0;
		this.fruitsEaten = 0;		
		...
		directionsSnakeIsGoing.clear();
		directionsSnakeIsGoing.add(Direction.North);		
	...
	public Direction getDirection() {
		return directionsSnakeIsGoing.peek();
	}
